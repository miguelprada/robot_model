...
{
  os << "    <link name=\"${prefix}" << link->name << "\">" << endl;
  if ( !!link->visual ) {
    os << "      <visual>" << endl;
...
void addChildJointNamesXML(boost::shared_ptr<const Link> link, ofstream& os)
{
  ...
  for (std::vector<boost::shared_ptr<Link> >::const_iterator child = link->child_links.begin(); child != link->child_links.end(); child++){
    ...
    os << "    <joint name=\"${prefix}" <<  (*child)->parent_joint->name << "\" type=\"" << jtype << "\">" << endl;
    os << "      <parent link=\"${prefix}" << link->name << "\"/>" << endl;
    os << "      <child  link=\"${prefix}" <<  (*child)->name << "\"/>" << endl;
...
void printTreeXML(boost::shared_ptr<const Link> link, string name, string file)
{
  ...
  os << "<robot name=\"" << name << "_xacro\"" << endl;
  ...
  os << "       xmlns:xacro=\"http://www.ros.org/wiki/xacro\">" << endl;

  os << "  <xacro:macro name=\"" << name << "\" params=\"prefix\">" << endl;
...
int main(int argc, char** argv)
{
  ...
  if (output_file == "") {
    output_file =  arobot_name + ".urdf.xacro";
  }
